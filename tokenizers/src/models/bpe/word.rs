...
impl Word {
    ...
    pub(super) fn add(&mut self, c: u64) {
        let (prev, next) = {
    ...
    pub(super) fn merge(&mut self, c1: u64, c2: u64, replacement: u64) -> Vec<(Pair, u64)> {
        let mut changes: Vec<(Pair, u64)> = vec![];
        let mut i = 0;
        loop {
    ...
    pub(super) fn merge_all(&mut self, merges: &HashMap<Pair, (u64, u64)>, dropout: Option<f64>) {
